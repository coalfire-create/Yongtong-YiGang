...
// 탐구 대그룹 내 세부 과목 카드 순서
const SCIENCE_CARD_ORDER = ["통합과학", "물리", "화학", "생명", "지구"];
// 강좌(+매칭된 강사커리큘럼)로부터 과탐 세부과목 판별
function sciSubjectOf(className: string, guidelineTitle?: string): string {
  const cn = (className || "").replace(/\s+/g, "");
  // 강좌명이 '통합과학/통과'(특강 아님)면 정규반(통합과학)으로 우선 처리
  if (/통합과학|통과/.test(cn) && !/특강/.test(cn)) return "통합과학";
  const s = ((guidelineTitle || "") + " " + cn).replace(/\s+/g, "");
...
const getSchoolRank = (name: string): number => {
  const sciIdx = SCIENCE_CARD_ORDER.indexOf(name);
  if (sciIdx !== -1) return 0.01 + sciIdx * 0.01; // Render science cards in order: 통합과학 -> 물리 -> 화학 -> 생명 -> 지구
...
const getG2ScienceClassScore = (className: string, teacherName: string): number => {
  const cn = (className || "").toUpperCase();
  const tn = teacherName || "";
  ...
  else if (cn.includes("화학") || cn.includes("물질과") || cn.includes("반응")) base = 300;
  else if (cn.includes("생명") || cn.includes("세포")) base = 400;

  // 2. 각 과목 내 세부 강사/강좌 순서 매칭
  if (base === 200) { // 물리
    if (tn.includes("유승진")) return base + 1;
    if (tn.includes("심규원")) return base + 2;
  }
  if (base === 300) { // 화학
    if (tn.includes("장해든누리")) return base + 1;
    if (tn.includes("김종인")) {
      if (cn.includes("물질")) return base + 2;
      return base + 3; // 화학 반응의 세계
    }
    if (tn.includes("변현수")) return base + 4;
  }
  if (base === 400) { // 생명
...
export function TimetableGallery({ category, filterTabs, summaryDivision, summaryTitle }: TimetableGalleryProps) {
  ...
  for (const tt of filtered) {
    ...
    const isG1OrG2Science = (category === "고등관-고1" || category === "고등관-고2") && (subj === "탐구" || SCIENCE_SUBJECTS.includes(subj));
    if (isG1OrG2Science) {
      if (isAllOrMathSciFilter) {
        subj = "탐구";
      } else {
    ...
    if (isG1OrG2Science && isAllOrMathSciFilter) {
      const sciSub = sciSubjectOf(tt.class_name || "", brochureMap.get(tt.id)?.title);
      instances.push({
        isUnion: false,
        targetSchool: sciSub,
      });
      ...
      if (isSpecialLecture) {
        ...
        const forceUnion = isG1OrG2Science && isAllOrMathSciFilter;
        instances.push({
...
function GroupCard({
  ...
  indexedTimetables.sort((a, b) => {
    const isG1ScienceCard = firstTt?.category === "고등관-고1" && (
      SCIENCE_CARD_ORDER.includes(title) || 
      SCIENCE_SUBJECTS.includes(firstTt?.subject || "")
    );
    if (isG1ScienceCard) {
    ...
    const isG2ScienceCard = firstTt?.category === "고등관-고2" && (
      SCIENCE_CARD_ORDER.includes(title) || 
      SCIENCE_SUBJECTS.includes(firstTt?.subject || "")
    );
    if (isG2ScienceCard) {
      const scoreA = getG2ScienceClassScore(a.tt.class_name || "", a.tt.teacher_name || "");
      const scoreB = getG2ScienceClassScore(b.tt.class_name || "", b.tt.teacher_name || "");
      if (scoreA !== scoreB) {
        return scoreA - scoreB;
      }
      return a.idx - b.idx;
